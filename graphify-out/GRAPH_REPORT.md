# Graph Report - athome-admin  (2026-06-21)

## Corpus Check
- 129 files · ~64,555 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 860 nodes · 1322 edges · 70 communities (55 shown, 15 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 8 edges (avg confidence: 0.81)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `8f453e8a`
...
8. `Этап 2: CRUD клиентов — Implementation Plan` - 16 edges
9. `Этап 4: Касания + экспорт для Claude — Implementation Plan` - 16 edges
10. `Design: Этап 6 — Панель + weekly_stats` - 16 edges
...
- `Shtab MVP Design Spec` --derives_from--> `Next.js 16 agent rules`  [EXTRACTED]
  docs/superpowers/specs/2026-06-15-shtab-mvp-design.md → AGENTS.md
...
Cohesion: 0.07
Nodes (43): registerFirstTrainer(), trainersExist(), ClientFilters(), ClientForm(), Props, ClientsList(), profileTone, statusTone (+35 more)

### Community 1 - "Auth, DB Schema & Client Queries"
Cohesion: 0.14
Nodes (17): NewClient, NewTouch, NewTrainer, NewWeeklyStat, Touch, TOUCH_TYPES, touches, TouchType (+9 more)
...
Cohesion: 0.19
Nodes (13): buildExportForSelection(), buildClaudeExport(), BuildClaudeExportResult, ClientForExport, ACTIVE_STALE_MEDIUM, EXPORT_SANITY_CASES, ExportSanityCase, computeTrigger() (+5 more)

### Community 5 - "App Shell & Auth Bootstrap"
Cohesion: 0.24
Nodes (6): MobileFab(), MobileTabBar(), NavLink(), Props, Sidebar(), TopBar()
...
Cohesion: 0.18
Nodes (11): groupAndSortTriggers(), TodayPage(), BoardEntry, BoardGroup, EMPTY_MODAL, ModalState, TodayBoard(), EmptyState() (+3 more)

### Community 10 - "Runtime Dependencies (package.json)"
Cohesion: 0.06
Nodes (49): commitImport(), CommitResult, previewImport(), PreviewResult, TxLike, coerceBool(), coerceDate(), coerceEnum() (+41 more)
...
Cohesion: 0.18
Nodes (10): {handlers, signIn, signOut, auth}, authEdgeConfig, RegisterResult, RegisterSchema, db, DEFAULT_TRAINER_SETTINGS, trainers, { auth } (+2 more)
...
Cohesion: 0.06
Nodes (39): requireTrainerId(), collectFieldErrors(), createClient(), createLead(), softDeleteClient(), updateClient(), weeklyStats, changeTrainerPassword() (+31 more)
...
Cohesion: 0.13
Nodes (20): CountersBlock(), DashboardPage(), SP, DashboardSanityPage(), NUMERIC_FIELDS, Props, WeeklyForm(), WeeklyHistoryTable() (+12 more)
...
Cohesion: 0.14
Nodes (13): PRIORITY_TILES, STATUS_LABELS, STATUS_ORDER, Client, ClientStatus, TrainerSettings, GroupKey, PRIORITY_ORDER (+5 more)
...
Cohesion: 0.17
Nodes (12): Sanity-проверка, Зачем, Как отмечать, Касания: концепция и работа, Модалка «Без личного факта», Пятничный ритуал (для чего это всё), Способ 1 — `/today`: «✓ Отметил», Способ 2 — `/clients/[id]`: «Отметить касание» (+4 more)

### Community 44 - "Community 44"
Cohesion: 0.22
Nodes (6): TriggerKind, TriggerPriority, DEFAULT_THRESHOLDS, SANITY_CASES, SanityCase, TODAY

### Community 45 - "Community 45"
Cohesion: 0.31
Nodes (9): /dev/export-sanity page, Пятничный ритуал, Инвариант: контакта нет в экспорте, Личный факт нужен для экспорта, Quick-touch с useOptimistic, Касание (touch), ERD: trainers/clients/touches/weekly_stats, Этап 4: Касания + экспорт — Plan (+1 more)
...
Cohesion: 0.25
Nodes (9): graphify skill registration, graphify pipeline, Step 0 - GitHub clone, Step 1 - Ensure graphify installed, Step 9 - Save manifest, cost, cleanup, report, /graphify add <url>, graphify.ingest module, graphify clone (single repo) (+1 more)
...
Cohesion: 0.28
Nodes (9): Step B0 - Check extraction cache, Gemini backend for semantic extraction, general-purpose subagent type, Part A - AST structural extraction, Part B - Semantic extraction via subagents, Part C - Merge AST + semantic, Step 3 - Extraction (AST + semantic), Step B2 - Parallel subagent dispatch (+1 more)
...
Cohesion: 0.29
Nodes (7): Next.js 16 agent rules, Tailwind 4 theme tokens, athome-pgdata volume, postgres service (postgres:16-alpine), Этап 1: Фундамент — Plan, Tech stack: Next.js 16 + React 19 + Drizzle + Auth.js v5, Register active only while trainers empty
...
Cohesion: 0.33
Nodes (7): 🔇 Тихий — страховка-приоритет 45+ дней, Этап 2: CRUD клиентов — Plan, Этап 3: Триггеры + Сегодня — Plan, computeTrigger pure function, CSV Import (papaparse + zod + transactional), SQL LEFT JOIN LATERAL for lastTouchDate, Shtab MVP Design Spec
...
## Knowledge Gaps
- **448 isolated node(s):** `Phase`, `SP`, `SP`, `CaseResult`, `TODAY` (+443 more)
...
- **Why does `requireTrainerId()` connect `Community 28` to `Auth, DB Schema & Client Queries`, `Claude Export Pipeline`, `Runtime Dependencies (package.json)`, `Edge Auth Proxy`, `Community 30`?**
  _High betweenness centrality (0.006) - this node is a cross-community bridge._
- **Why does `Button` connect `Runtime Dependencies (package.json)` to `Client UI Forms & Lists`, `Auth, DB Schema & Client Queries`, `App Shell & Auth Bootstrap`, `Clients, Dashboard & Settings Pages`, `Community 28`, `Community 30`?**
  _High betweenness centrality (0.006) - this node is a cross-community bridge._
